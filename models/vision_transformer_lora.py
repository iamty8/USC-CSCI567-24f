...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # self.adapter1 = Adapter(config=adapter_config, d_model=dim)
        # self.adapter2 = Adapter(config=adapter_config, d_model=dim)
    ...
    def forward(self, x, return_attention=False):
        ...
        x = x + y
        if self.config is not None and self.config.ffn_option == 'parallel':
            adapt_x = self.adapter(x, add_residual=False)
        residual = x
        x = self.norm2(x)
        x = self.mlp(x)
        if self.config is not None and self.config.ffn_adapt:
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, adapter_config=None, **kwargs):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, config=adapter_config)
